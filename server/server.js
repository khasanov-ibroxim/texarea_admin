...
// MUHIM: CORS MIDDLEWARE - SESSION DAN OLDIN!
app.use(cors({
  origin: 'http://localhost:3001',
  credentials: true,
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS'],
  allowedHeaders: ['Content-Type', 'Authorization'],
  exposedHeaders: ['Set-Cookie'], // ← MUHIM!
}));
...
// Session middleware - CORS dan KEYIN!
app.use(session({
  ...
  cookie: {
    ...
    httpOnly: true,          // XSS himoya
    sameSite: 'lax',        // CSRF himoya (strict emas!)
    maxAge: 24 * 60 * 60 * 1000, // 24 soat
    path: '/',
    domain: undefined,       // localhost uchun domain o'rnatmaslik
  }
}));
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(`🔐 CORS:      ENABLED (credentials: true)`);
      console.log(`┌─────────────────────────────────────────────┐\n`);
    });
  } catch (error) {
